...
from variables import (width, height, GAMESPEED, environmentObjectsPerBiome, IMAGES, plainSpritesPerBiome,
                       randomDamagingTrapsPerBiome, notRespawningEnvironmentObjectsPerBiome)
from foe import foe
from definitions import skip, percentChance, greater, lesser
...
class room:
    def __init__(self, coordinate, biome, **extra):
        ...
        if self.addsResources:
            ...
            try:
                for i in range(random.choice([0, 0, 0, 0, 0, 1, 1, 1, 2])):
                    print((self.biome, self.coordinate[2]), notRespawningEnvironmentObjectsPerBiome.keys())
                    print(notRespawningEnvironmentObjectsPerBiome[(self.biome, self.coordinate[2])])
    ...
    def getUnavailableCells(self):
        ...
        for cell in self.cells:
            ...
            print(cell[2], self.yBoundaries, 'why')

            if cell[2] <= self.yBoundaries:
                if cell[3] > self.yBoundaries:
                    cell[2] = self.yBoundaries
    ...
    def getLocationInCell(self, pyRect, hitbox=None):
        ...
        minX = int(hitbox.width / 2 + cellUsed[0])
        maxX = int(cellUsed[1] - hitbox.width / 2)
        minY = int(cellUsed[2] + hitbox.height / 2)
        maxY = int(cellUsed[3] - hitbox.height / 2)

        try:
            xCoord = random.randint(minX, maxX)

        except ValueError:
            xCoord = lesser(greater(cellUsed[0] + width / 40, self.yBoundaries + hitbox.height / 2),
        ...
        try:
            ...
            yCoord = lesser(greater(cellUsed[2] + height / 40, self.yBoundaries + hitbox.height / 2),
