...
class world:
    ...
    def addTopDesertCaveLayerSpecialRooms(self, biome):
        ...
        for place in biome:
            if place.biome == 'desert' and place.difficulty == -2:
                huntingSpider = foe('desertCaveSpider', width / 4, height / 4, place.coordinate.copy(), hp=100)
                huntedFly = foe('desertCaveLargeFly', width / 2, height / 2, place.coordinate.copy(), hp=100)
                huntingSpider.unusualTarget = huntedFly
                huntedFly.unusualTarget = huntingSpider
                huntedFly.aggressionRadius = float('inf')
                place.foes = [huntingSpider, huntedFly]
