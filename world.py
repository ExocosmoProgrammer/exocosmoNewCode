...
class world:
    def __init__(self, **extra):
        ...
        shipRooms = [room([0, 0, 10], 'ship'),
                     ...
                     room([-1, 10, 10], 'gauntlet',
                          teleporters=[teleporter(width / 2, height / 2, 'shelterEntrance.png',
                                                  [-1, 5, 10],
                                                  destinationXandY=[width / 2, height * 3 / 4])]),

                     room([-1, 11, 10], 'gauntlet',
                     ...
                     room([0, 12, 10], 'gauntlet',
                          ...
                          foes=[foe('gauntletFly', width / 5, height / 2, [0, 12, 10],
                                      deathEffect='proRoom().locks = True; rooms.rooms[(-1, 13, 10)].damagingTraps = '
                                                  ...
                                                  'displayVars.screenShakeDuration = '
                                                  'greater(displayVars.screenShakeDuration, 100)',
                     ...
                     room([0, 14, 10], 'gauntlet',
                          foes=[foe('gauntletFly', width / 5, height / 2, [0, 14, 10],
                                    ...
                                    deathEffect='rooms.rooms[(-2, 12, 10)].damagingTraps=[damagingTrap("aFire.png",200,'
                                                ...
                                                ' [foe("gauntletFly", width / 2, height / 2, [-1, 11, 10], '
                                                'deathEffect="rooms.rooms[(-2, 12, 10)].damagingTraps=[];'
                                                'rooms.rooms[(-2, 12, 10)].specialRects = {}; '
                                                'proRoom().locks=True;'
                                                'displayVars.screenShakeDuration = '
                                                'greater(displayVars.screenShakeDuration, 100)   ", '
                                                'spawnsOnDefeat=[foe("gauntletKamikaze", width / 2, height / 2, '
                                                '[-1, 11, 10], duration=1000, '
                                                'deathEffect = "pro.heal(80); '
                                                'pro.getPotion(); proRoom().locks = False")])]; '
                                                'displayVars.screenShakeDuration = '
                                                'greater(displayVars.screenShakeDuration, 100)',
                                    spawnsOnDefeat=[foe('gauntletKamikaze', width * i / 16,
                                                        height * 7 / 32,
                                                        [0, 14, 10], hp=float('inf'),
                                                        duration=350, spawnDelay=100 * i,
                                                        fireDuration=210) for i in range(1, 16)] + \
                                                   [foe('gauntletKamikaze', width * 15 / 16,
                                                        height * (i / 8 + 3 / 32),
                                                        [0, 14, 10], hp=float('inf'), duration=200,
                                                        spawnDelay=100 * (i + 16),
                                                        fireDuration=210) for i in range(1, 8)] + \
                                                   [foe('gauntletKamikaze',
                                                        width * (16 - i) / 16, height * 31 / 32,
                                                        [0, 14, 10], hp=float('inf'), duration=200,
                                                        spawnDelay=100 * (i + 23),
                                                        fireDuration=210) for i in range(1, 16)] + \
                                                   [foe('gauntletKamikaze', width / 16,
                                                        height * (35 / 4 - i) / 8,
                                                        [0, 14, 10], hp=float('inf'), duration=200,
                                                        spawnDelay=100 * (i + 39),
                                                        fireDuration=210) for i in range(1, 8)] + \
                                                   [foe('gauntletKamikaze', width / 16, height * 7 / 32,
                                                        [0, 14, 10], hp=float('inf'), duration=200,
                                                        deathEffect='pro.heal(80); proRoom().locks = False',
                                                        spawnDelay=4600, fireDuration=210)])]),

                     room([-3, 12, 10], 'gauntlet', usuallyLocks=True,
                          foes=[foe('gauntletMiniboss', width / 2, height / 2, [-3, 12, 10]),
                                foe('gauntletKamikaze', width / 2, height / 8, [-3, 12, 10],
                                    duration=20000)]),

                     room([-4, 12, 10], 'gauntlet', locks=True, usuallyLocks=True,
                          foes=[foe('gauntletFly', width / 2, height / 2, [-4, 12, 10],
    ...
    def addTopDesertCaveLayerSpecialRooms(self, biome):
        ...
        middleLumisLakeCoord = (int((rightLumisLakeXCoord + leftLumisLakeXCoord) / 2),
                                int((topLumisLakeYCoord + bottomLumisLakeYCoord) / 2), lumisLakeCoordinates[0][2],)
        ...
        self.rooms[middleLumisLakeCoord].background = 'arenaForContainer.png'
        self.rooms[middleLumisLakeCoord].environmentObjects = []
        self.rooms[middleLumisLakeCoord].foes = [foe('container', width / 2, height / 3,
                                                     list(middleLumisLakeCoord))]
        self.rooms[middleLumisLakeCoord].oneTimeEntranceEffect =  ('rooms.rooms[(1, 0, 10)].teleporters.append('
                                                                  'teleporter(width * 3 / 32, height * 7 / 32, '
                                                                  f'"shelterEntrance.png", {list(middleLumisLakeCoord)}, '
